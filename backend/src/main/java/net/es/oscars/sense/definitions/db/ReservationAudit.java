...
public class ReservationAudit {
  ...
  /**
   * Find any NSI reservations in the database that are not in the current list
   * returned from the NSA.We need to be tricky since removing a reservation also
   * removes our ability to indicated a change that requires a new topology model
   * to be generated.
   *
    ...
    for (Reservation reservation : reservationService.get()) {
      ...
      if (auditable.contains(cid)) {
        ...
        if (reservation.getLifecycleState() != LifecycleStateEnumType.TERMINATED) {
          log.debug("[ReservationAudit] transitioning cid = {}, from {} to {}", reservation.getConnectionId(),
              reservation.getLifecycleState(), LifecycleStateEnumType.TERMINATED);
          ...
          log.debug("[ReservationAudit] deleting cid = {}, lifecycleState = {}", reservation.getConnectionId(),
