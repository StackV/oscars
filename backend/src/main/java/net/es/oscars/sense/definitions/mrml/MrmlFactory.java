...
// import net.es.oscars.sense.tools.XmlDate;
...
// public MrmlFactory(NmlModel nml, SwitchingSubnetModel ssm, String topologyId)
...
// this.topology = nml.getTopology(topologyId).orElseThrow(new
...
// /**
// * Returns an MRML Ontology containing topologyResource for the specified
// * topologyResource.
// *
...
// // Add bidirectional ports using common attributes from unidirectional
// members.
...
// private static Resource createResource(OntModel model, String uri, Resource
// type) {
...
// model.add(model.createStatement(nmlTopology, Nml.existsDuring,
// createLifetime(model)));
...
// // Instead of using the NML document version we make our own based on the
// most
// // recent of the topology
// // document or connection discovery.
// try {
// long version = (nml.getLastDiscovered() < ssm.getVersion()) ?
// ssm.getVersion() : nml.getLastDiscovered();
// model.add(
// model.createStatement(nmlTopology, Nml.version,
// XmlDate.longToXMLGregorianCalendar(version).toXMLFormat()));
// } catch (DatatypeConfigurationException ex) {
// log.error("createTopolgyResource: failed to convert internal version to
// XML.", ex);
// model.add(model.createStatement(nmlTopology, Nml.version,
// topology.getVersion().toXMLFormat()));
// }
...
// return Strings.isNullOrEmpty(topology.getName()) ? topology.getId() :
// topology.getName();
// }

// private Resource createLifetime(OntModel model) {
// Resource res = model.createResource(Nml.Lifetime);
// if (topology.getLifetime() == null || topology.getLifetime().getStart() ==
// null) {
...
// if (topology.getLifetime() != null && topology.getLifetime().getEnd() !=
// null) {
...
// res.addProperty(Nml.address, XmlUtilities.jaxbToString(NmlAddress.class,
// location.getAddress()));
// }
...
// * @throws IllegalArgumentException
// */
// private Map<String, Resource> createBidirectionalPorts(OntModel model) throws
// IllegalArgumentException {
// Map<String, Resource> biPorts = new HashMap<>();

// Resource topologyResource = model.getResource(topologyId);

// nml.getPorts(topologyId,
// Orientation.bidirectional).values().stream().forEach(p -> {
...
// String postfix = l.getLabeltype().substring(l.getLabeltype().lastIndexOf("#")
// + 1);
// Resource label = createResource(model, p.getId() + ":" + postfix,
// Nml.LabelGroup);
...
// Resource bw = createResource(model, p.getId() + ":BandwidthService",
...
// bw.addLiteral(Mrs.granularity,
// p.getGranularity().orElse(nml.getDefaultGranularity()));
...
// // p.getAvailableCapacity().ifPresent(c ->
// bw.addLiteral(Mrs.availableCapacity,
// // c));
// p.getReservableCapacity().ifPresent(c ->
// bw.addLiteral(Mrs.reservableCapacity, c));
// p.getIndividualCapacity().ifPresent(c ->
...
// private Map<String, Resource> createServiceDefinition(OntModel model) throws
// IllegalArgumentException {
...
// Resource sd = createResource(model, serviceDefinition.getId(),
...
// private Map<String, Resource> createSwitchingService(OntModel model) throws
// IllegalArgumentException {
...
// createBidirectionalPortsFromConnections(OntModel model)
// throws IllegalArgumentException {
// Map<String, Resource> biPorts = new HashMap<>();

// log.debug("[MrmlFactory] createBidirectionalPortsFromConnections for
// topologyId {}", topologyId);

// nml.getPorts(topologyId, Orientation.child).values().stream().forEach(p -> {
// Resource parentPort = model.getResource(p.getParentPort().get());

// log.debug("[MrmlFactory] creating child port {}, parentPort {}, resource {}",
// p.getId(), p.getParentPort().get(),
// parentPort);
...
// final Resource existsDuring = createLifetime(model,
// p.getNmlExistsDuringId().get(), p.getStartTime(),
// p.getEndTime());
...
// bw.addLiteral(Mrs.minimumCapacity, p.getMinimumCapacity().orElse(1L));
// p.getUsedCapacity().ifPresent(c -> bw.addLiteral(Mrs.usedCapacity, c));
// p.getAvailableCapacity().ifPresent(c -> bw.addLiteral(Mrs.availableCapacity,
// c));
// p.getReservableCapacity().ifPresent(c ->
// bw.addLiteral(Mrs.reservableCapacity, c));
// p.getIndividualCapacity().ifPresent(c ->
...
// private Map<String, Resource> createSwitchingSubnet(OntModel model) throws
// IllegalArgumentException {
// Map<String, Resource> ssCollection = new HashMap<>();
...
// // Create the NML switching service.
// Resource ssr = createResource(model, switchingSubnet.getId(),
// Mrs.SwitchingSubnet);
...
// Resource existsDuring = createLifetime(model,
// switchingSubnet.getExistsDuringId(),
...
// private Resource createLifetime(OntModel model, String id, Optional<Long>
// startTime, Optional<Long> endTime) {
...
// /**
// * Resource lifeTime = model.getResource(id); if (lifeTime != null) {
// * log.debug("[createLifetime] found Lifetime resource {}, {}",
// * lifeTime.getURI()); return lifeTime; }
// **/
...
// res.addProperty(Nml.start, start.toXMLFormat());
// } catch (DatatypeConfigurationException ex) {
// log.error("[MrmlFactory] failed to create startTime xmlGregorianCalendar,
// {}", ex);
// }
// });
...
// res.addProperty(Nml.end, end.toXMLFormat());
// } catch (DatatypeConfigurationException ex) {
// log.error("[MrmlFactory] failed to create endTime xmlGregorianCalendar, {}",
// ex);
// }
// });
