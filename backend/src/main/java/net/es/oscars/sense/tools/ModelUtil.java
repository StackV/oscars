...
public class ModelUtil {
    ...
    public static final String[] supported = { "turtle", "ttl" };
    ...
    public static Resource getResourceOfType(Model model, Resource res, Resource resType) {
        String sparql = String.format(
                "SELECT ?resource WHERE {?resource a ?type. FILTER($resource = <%s> && $type = <%s>)}", res, resType);
        ResultSet r = ModelUtil.sparqlQuery(model, sparql);
    ...
    public static Resource getParentBidirectionalPort(Model model, Resource res) {
        String sparql = String.format("SELECT ?resource WHERE { ?resource a <" + Nml.BidirectionalPort
                + ">. ?resource <" + Nml.hasBidirectionalPort + "> <%s> }", res);
        ResultSet r = ModelUtil.sparqlQuery(model, sparql);
    ...
    public static ResultSet sparqlQuery(Model model, String sparqlStringWithoutPrefix) {
        String sparqlString = "prefix sd:    <http://schemas.ogf.org/nsi/2013/12/services/definition#>\n"
