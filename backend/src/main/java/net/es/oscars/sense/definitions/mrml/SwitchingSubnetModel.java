...
// import net.es.oscars.sense.definitions.db.ConnectionMapService;
...
// /**
// * Create MRML SwitchingSubnet model from stored NSI reservations and
// connection
// * maps.
// *
// * @author hacksaw
...
// // Will contain a map of serviceHolder indexed by SwitchingService
// identifier.
...
// NmlModel nml, String topologyId) throws IllegalArgumentException {
...
// // Pull out ServiceDefinitions from SwtichingServices and link Bidirectional
// // ports.
...
// log.info("[SwitchingSubnetModel] processing SwitchingService {}",
// ss.getId());

// ss.getAny().stream().filter((object) -> (object instanceof
// JAXBElement)).map((object) -> (JAXBElement) object)
// .filter((jaxb) -> (jaxb.getValue() instanceof ServiceDefinitionType))
// .map((jaxb) -> (ServiceDefinitionType) jaxb.getValue()).forEachOrdered((sd)
// -> {
// ServiceDefinitionType servdef = serviceDefinitions.get(sd.getId());
// // We have a ServiceDefinition against this SwitchingService.
// ServiceHolder sh = new
// ServiceHolder.ServiceHolderBuilder().serviceType(servdef.getServiceType())
// .serviceDefinition(servdef).switchingService(ss).build();
// serviceHolder.put(ss.getId(), sh);
// });

// // Now link all the bidirection port members back to their containing
// // SwitchingService.
...
// /**
// * Convert the NSI CS reservations into MRML SwitchingSubnet elements and link
// * to parent SwitchingService.
// */
...
// reservationService.getByTopologyId(topologyId)) {
...
// log.info("[SwitchingSubnetModel] new version discovered reservation cid =
...
// log.info("[SwitchingSubnetModel] skipping reservation cid = {},
// reservationState = {}",
...
// // One last test before processing this reservation... if this is a child
// // reservation,
// // and the parent reservation is present in the database, then we skip the
// child
// // reservation. The oparent reservation will only be present if the src and
// dst
// // STP
// // are in our topology. If either of the src or dst were not in out topology
// // then we
...
// Reservation parent = reservationService.get(reservation.getProviderNsa(),
// reservation.getParentConnectionId());
// if (parent != null) {
// log.info("[SwitchingSubnetModel] skipping child reservation cid = {},
// lifecycleState = {}, parentId = {}",
// reservation.getConnectionId(), reservation.getLifecycleState(),
...
// connMap =
// Optional.ofNullable(connectionMapService.getByDescription(reservation.getDescription()));
// }
...
// P2PServiceBaseType p2ps =
// CsParser.getInstance().xml2p2ps(reservation.getService());
// Optional<Long> startTime = reservation.getStartTime() == 0 ? Optional.empty()
// : Optional.of(reservation.getStartTime());
// Optional<Long> endTime = reservation.getEndTime() == Long.MAX_VALUE ?
// Optional.empty()
// : Optional.of(reservation.getEndTime());
// Optional<Long> capacity = Optional.of(p2ps.getCapacity() * 1000000);

// Optional<NmlPort> srcChildPort = createChildPort(reservation.getTopologyId(),
// reservation.getConnectionId(),
// p2ps.getSourceSTP(), connMap, startTime, endTime, capacity);

// // No nml port if consolidation with parent port is not possible.
// if (!srcChildPort.isPresent()) {
// log.error("[SwitchingSubnetModel] Could not find parent port for STP {}",
// p2ps.getSourceSTP());
// continue;
// }

// Optional<NmlPort> dstChildPort = createChildPort(reservation.getTopologyId(),
// reservation.getConnectionId(),
// p2ps.getDestSTP(), connMap, startTime, endTime, capacity);
...
// // We can only build a SwitchingSubnet if this service contains two endpoints
// in
// // the same network.
// if
// (!srcChildPort.get().getTopologyId().equalsIgnoreCase(dstChildPort.get().getTopologyId()))
// {
// log.error("[SwitchingSubnetModel] Reservation using STP on two different
// networks src = {}, dst = {}",
...
// Optional<String> srcSSid =
// createSwitchingSubnet(srcChildPort.get().getParentPort().get(), serviceType);
// Optional<String> dstSSid =
// createSwitchingSubnet(dstChildPort.get().getParentPort().get(), serviceType);

// log.info("[SwitchingSubnetModel] srcSSid = {}, dstSSid = {}", srcSSid,
// dstSSid);

// if (!srcSSid.orElse("srcSSid").equalsIgnoreCase(dstSSid.orElse("dstSSid"))) {
// log.error(
// "[SwitchingSubnetModel] Reservation using STP from two different
// SwitchingService src = {}, dst = {}",
...
// } else {
// nssId = NmlSwitchingSubnet.id(reservation.getTopologyId(),
// ConnectionId.strip(reservation.getConnectionId()));
// }
...
// nss.setTag("connectionId=" + topologyId + ":cid+" +
// ConnectionId.strip(reservation.getConnectionId()));
// holder.getSwitchingSubnets().add(nss);
// log.info("[SwitchingSubnetModel] adding SwitchingSubnet = {}", nss.getId());
// } catch (JAXBException ex) {
// log.error("Could not parse P2PS structure for conenctionId = {}",
// reservation.getConnectionId(), ex);
// }
// } else {
// log.error("Unknown serviceType for conenctionId = {}, serviceType = {}",
// reservation.getConnectionId(),
...
// /**
// * Create a child port under a the parent bidirectional port by using NSI
...
// * @return
// */
// private Optional<NmlPort> createChildPort(String topologyId, String
// connectionId, String stp,
// Optional<ConnectionMap> connMap, Optional<Long> startTime, Optional<Long>
// endTime, Optional<Long> capacity) {
...
// childPortExistsDuringId =
// Optional.ofNullable(stpMapping.get().getNmlExistsDuringId());
// } else {
// childPortId = simpleStp.getMrmlId() + ":cid+" +
// ConnectionId.strip(connectionId);
// }
...
// NmlPort childPort =
// NmlPort.builder().id(childPortId).topologyId(topologyId).mrsBandwidthId(childPortBwId)
// .mrsLabelId(childPortLabelId).nmlExistsDuringId(childPortExistsDuringId).name(Optional.of(connectionId))
// .orientation(Orientation.child).parentPort(Optional.of(stpParent.getId())).encoding(stpParent.getEncoding())
// .interfaceMTU(stpParent.getInterfaceMTU()).type(stpParent.getType()).granularity(stpParent.getGranularity())
// .maximumCapacity(capacity) // This would be maximumCapacity of parent for
// soft cap service.
// .minimumCapacity(stpParent.getMinimumCapacity()) //
// .reservableCapacity(capacity).availableCapacity(capacity).individualCapacity(stpParent.getIndividualCapacity())
...
// private Optional<String> createSwitchingSubnet(String portId, String
// serviceType) {
// // Now build the SwitchingSubnet.
...
// log.info("[SwitchingSubnetModel] checking source SwitchingService {}",
// t.getId());
...
// log.info("[SwitchingSubnetModel] Matched source SwitchingService {}",
// t.getId());
// }
// }
